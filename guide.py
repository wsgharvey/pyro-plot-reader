...
class Guide(nn.Module):
    ...
    def forward(self, observed_image=None, print_params=False):
        ...
        if self.multi_bar_charts:
            ps = self.program_step("num_bar_charts", prev_sample_value)
        ...
        if self.random_img_dim:
            mode, certainty = self.program_step("img_width", prev_sample_value)
            prev_sample_value = pyro.sample("img_width",
                                            ...
                                            mode*75 + 25,
                                            certainty)
            mode, certainty = self.program_step("img_height", prev_sample_value)
        ...
        if self.random_layout:
            ps = self.program_step("no_spines", prev_sample_value)
        ...
        if self.multi_bar_charts:
            for bar_chart in range(num_bar_charts):
                ...
                for _ in range(num_bars):
                    mode, certainty = self.program_step(sample_name,
                                                     prev_sample_value)
